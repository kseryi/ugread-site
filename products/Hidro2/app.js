...
(function () {
  ...
  const I18N = {
    uk: {
      ...
      status_press_insufficient_force: 'СИЛА НЕДОСТАТНЯ (F₂ < F_тяж)',
    ...
    en: {
      ...
      status_press_insufficient_force: 'INSUFFICIENT FORCE (F₂ < F_g)',
  ...
  function setupCanvasInteractions() {
    ...
    let lastPointerPos = { x: 0, y: 0 };
    let dragStartY = 0;
    let dragStartForce1 = 0;
    let dragStartPiston1H = 90;
      ...
      if (state.mode === 'vessels') {
        ...
        if (pressGeom) {
          ...
          if (state.pressSubmode === 'jack') {
            ...
            // Check if clicking on workpiece on large piston platen to cycle/select load
            const { piston2Center, w2 } = pressGeom;
            const platenY = bottomY - state.piston2Height - 24;
            if (Math.abs(pos.x - piston2Center) <= w2 / 2 + 25 && pos.y >= platenY - 85 && pos.y <= platenY + 20) {
              const currentIdx = LOADS_DB.findIndex(l => l.id === state.selectedLoad.id);
              const nextIdx = (currentIdx + 1) % LOADS_DB.length;
              state.selectedLoad = LOADS_DB[nextIdx];
              populateLoadsUI();
              updatePressPhysics();
              return;
            }

            // Standard Press: small piston grab / direct pressing with smooth force buildup
            if (Math.abs(pos.x - piston1Center) <= w1 / 2 + 40 && pos.y >= bottomY - 260 && pos.y <= bottomY + 20) {
              state.isDraggingPiston1 = true;
              dragStartPiston1H = state.piston1Height;
              dragStartForce1 = state.force1;
              dragStartY = pos.y;
              canvas.style.cursor = 'ns-resize';

              // Apply immediate responsive step right away on click
    ...
    const handlePointerMove = (e) => {
      ...
      if (!isPointerDownOnCanvas) {
        ...
        if (state.mode === 'press' && state.pressSubmode === 'standard') {
          const pressGeom = getPressGeometry();
          if (pressGeom) {
            const { piston1Center, w1, piston2Center, w2, piston1Left, bottomY } = pressGeom;
            const stdValveX = piston1Left + w1 + 35;
            const stdValveY = bottomY - 18;
            const platenY = bottomY - state.piston2Height - 24;
            ...
            } else if (Math.abs(pos.x - piston2Center) <= w2 / 2 + 25 && pos.y >= platenY - 85 && pos.y <= platenY + 20) {
              canvas.style.cursor = 'pointer';
            } else if (Math.abs(pos.x - stdValveX) <= 26 && Math.abs(pos.y - stdValveY) <= 26) {
              canvas.style.cursor = 'pointer';
            } else {
  ...
  // Calculate real-time instantaneous piston tracking, force buildup, and incompressible volume conservation
  function applyDirectPistonDisplacement(mouseY) {
    const pressGeom = getPressGeometry();
    if (!pressGeom) return;

    const dy = mouseY - dragStartY; // positive downward displacement
    // 1 pixel of downward drag corresponds to ~3.5 N of applied force F1
    const newForce = Math.max(0, Math.min(1000, dragStartForce1 + dy * 3.5));
    state.force1 = Math.round(newForce);

    if ($('force1-slider')) $('force1-slider').value = state.force1;
    if ($('force1-input')) $('force1-input').value = state.force1;

    // Apply physics directly and instantaneously
    updatePressPhysics(true);
  }
  ...
  // Calculate maximum oil volume capacity of the Jack Ram cylinder based on S2 and 85mm max stroke
  function getJackRamMaxVolume() {
    const S2 = Math.PI * Math.pow(state.piston2Dia / 2, 2);
    // Max travel from bottom 90mm to top stop 175mm is 85mm.
    // liftFromJack = (V * 1000) / S2 * 7.5 = 85mm => V = (85 * S2) / 7500
    return Math.max(10, (85 * S2) / 7500);
  }
  ...
  function updatePressPhysics(forceDirect = false) {
    ...
    if (state.pressSubmode === 'standard') {
      const contactH2 = 134; // mm: height where platen brings workpiece to contact upper anvil
      const maxSquash = 14;  // mm: compression travel (134 to 148 mm)
      const maxH2 = contactH2 + maxSquash; // 148 mm: rigid mechanical stop

      if (state.force1 > 0) {
        if (netForce >= 0) {
          // Output force F2 overcomes load weight F_тяж!
          const requiredF1 = loadWeight > 0 ? (loadWeight / Math.max(1, areaRatio)) : 0;
          const maxF1 = 1000;
          const availableExcessF1 = Math.max(15, maxF1 - requiredF1);
          // Fraction of force above required threshold (0 at threshold, 1.0 at maximum force)
          const excessRatio = state.force1 >= requiredF1 ? Math.min(1.0, (state.force1 - requiredF1) / availableExcessF1) : 0;

          // Lift from base 90mm to contact 134mm (44mm lift)
          // Then compress from 134mm to 148mm (14mm compression)
          const liftStroke = 44 * Math.min(1.0, state.force1 / Math.max(1, requiredF1));
          const squashStroke = 14 * Math.min(1.0, excessRatio * 2.2);
          const rawTargetP2H = 90 + liftStroke + squashStroke;
          ...
          if (rawTargetP2H >= contactH2) {
            ...
            clamped = compression >= 0.95;
          }

          state.targetPiston2Height = Math.min(maxH2, clampedH2);
          state.isPressClamped = clamped;
          state.pressCompression = compression;

          // Incompressible fluid constraint on small piston:
          // Small piston descends from 90 mm down to 20 mm (70 mm maximum stroke)
          const liftFraction = (state.targetPiston2Height - 90) / (maxH2 - 90);
          state.targetPiston1Height = Math.max(20, 90 - liftFraction * 70);

        } else {
          // F2 < loadWeight: Load is too heavy for applied force F1.
          // Large piston cannot lift against the heavy load (stays at 90 mm).
          // Small piston meets stiff hydraulic resistance against incompressible fluid.
          state.targetPiston2Height = 90;
          const requiredF1 = loadWeight / Math.max(1, areaRatio);
          const stiffnessDeflection = Math.min(4, (state.force1 / Math.max(1, requiredF1)) * 4);
          state.targetPiston1Height = 90 - stiffnessDeflection;
          state.isPressClamped = false;
          state.pressCompression = 0;
        }
      } else {
        // Idle rest state (force1 == 0)
        state.targetPiston1Height = 90;
        state.targetPiston2Height = 90;
        state.isPressClamped = false;
        state.pressCompression = 0;
      ...
      // Jack Mode: Piston 2 height determined by displaced oil in Ram & Incompressibility
      const maxRamVol = getJackRamMaxVolume();
      state.jackOilVolumeInRam = Math.max(0, Math.min(maxRamVol, state.jackOilVolumeInRam));
      const liftFromJack = (state.jackOilVolumeInRam * 1000) / S2 * 7.5;
      state.targetPiston2Height = Math.min(175, 90 + liftFromJack);
      state.isJackAtMaxStroke = (state.targetPiston2Height >= 174.5 || state.jackOilVolumeInRam >= maxRamVol - 0.1);
    }
  ...
  function triggerStandardPressStep() {
    ...
    state.force1 = Math.min(1000, state.force1 + 45);
  ...
  // Jack lever action: continuous time-based mechanical pumping cycle with incompressibility check
  function pumpJackHandle() {
    if (state.jackPumpPhase !== 'idle' || state.isJackDraining) return;
    if (state.jackReservoirVolume <= 1) return; // empty reservoir

    const S1 = Math.PI * Math.pow(state.piston1Dia / 2, 2);
    const maxRamVol = getJackRamMaxVolume();
    const remainingRamCapacity = Math.max(0, maxRamVol - state.jackOilVolumeInRam);

    // If cylinder reached top mechanical stop, fluid is incompressible (no further volume can enter)
    if (remainingRamCapacity <= 0.05) {
      state.isJackAtMaxStroke = true;
      state.jackBypassReliefActive = true;
      // Visual feedback: handle is stiff/blocked, relief bypass triggers, NO oil drained from reservoir
      state.jackPumpPhase = 'pumping';
      state.jackPumpTimer = 0;
      state.jackPumpDuration = 0.32;
      state.jackPendingStrokeVol = 0;
      state.jackActualTransferredVol = 0;
      updateTelemetry();
      renderCalculations();
      return;
    }

    state.jackBypassReliefActive = false;
    const nominalStrokeVol = ((S1 * 55) / 1000) * 1.7; // ml per full stroke
    const strokeVol = Math.min(state.jackReservoirVolume, remainingRamCapacity, nominalStrokeVol);
    ...
    if (state.mode === 'vessels') {
      ...
      if (state.pressSubmode === 'standard') {
        if (state.isHoldingPressButton) {
          ...
          state.force1 = Math.min(1000, state.force1 + forceIncrement);
          $('force1-slider').value = Math.round(state.force1);
          $('force1-input').value = Math.round(state.force1);
          updatePressPhysics();
        }

        const smoothFactor = state.isDraggingPiston1 ? 1.0 : (1 - Math.exp(-18 * effectiveDt));
        ...
        if (state.jackPumpPhase === 'pumping') {
          ...
          if (progress <= 0.46) {
            ...
            const maxAngle = state.jackPendingStrokeVol === 0 ? -8 : -24; // Stiff resistance if full
            state.jackHandleAngle = maxAngle * ease;
            state.targetPiston1Height = 90 - (state.jackPendingStrokeVol === 0 ? 12 : 55) * ease;
            state.dischargeValveOpen = state.jackPendingStrokeVol > 0;
            state.suctionValveOpen = false;

            if (!state.jackStrokeOilTransferred && progress >= 0.44) {
              const maxVol = getJackRamMaxVolume();
              const roomLeft = Math.max(0, maxVol - state.jackOilVolumeInRam);
              const actualVol = Math.min(state.jackPendingStrokeVol, roomLeft);
              state.jackActualTransferredVol = actualVol;
              state.jackOilVolumeInRam += actualVol;
            ...
            // Upstroke: Lever handle returns up, fresh oil drawn from reservoir ONLY if oil was transferred
            const t = (progress - 0.46) / 0.54;
            const ease = (1 - Math.cos(t * Math.PI)) / 2;
            const maxAngle = state.jackPendingStrokeVol === 0 ? -8 : -24;
            state.jackHandleAngle = maxAngle * (1 - ease);
            state.targetPiston1Height = 90 - (state.jackPendingStrokeVol === 0 ? 12 : 55) * (1 - ease);
            state.dischargeValveOpen = false;
            state.suctionValveOpen = (state.jackActualTransferredVol || 0) > 0.05;

            if (!state.jackReservoirDeducted && progress >= 0.7) {
              const deduct = state.jackActualTransferredVol || 0;
              if (deduct > 0) {
                state.jackReservoirVolume = Math.max(0, state.jackReservoirVolume - deduct);
      ...
      // Smooth Analog Glycerin-Damped Manometer Needle Angle (spikes when clamped or at max stroke)
      const pressFraction = (state.isPressClamped || state.isJackAtMaxStroke || state.jackBypassReliefActive)
        ? 0.95 
        : Math.min(1.0, state.force1 / 500);
  ...
  function updateTelemetry(totalDiff = 0) {
    if (state.mode === 'vessels') {
      ...
      const isMaxClampedOrStroke = state.isPressClamped || state.isJackAtMaxStroke || state.jackBypassReliefActive;
      const effectivePressureKPa = isMaxClampedOrStroke ? (pressurePistonKPa > 0 ? pressurePistonKPa * 3.5 : 850) : pressurePistonKPa;
      ...
      $('telem-sub-3').textContent = isMaxClampedOrStroke ? (state.lang === 'uk' ? 'P_max (Запобіжний клапан)' : 'P_max (Safety Relief)') : 'P = F₁ / S₁';
      ...
      if (state.isJackDraining) {
      ...
      } else if (state.isJackAtMaxStroke && state.pressSubmode === 'jack') {
        $('telem-val-6').textContent = I18N[state.lang].status_jack_max_stroke;
        $('telem-val-6').className = 'telemetry-value highlight-emerald';
        $('telem-sub-6').textContent = state.lang === 'uk'
          ? `h₂ = 175 мм (макс) • ΔV = 0 (Рідина нестислива)`
          : `h₂ = 175 mm (max) • ΔV = 0 (Incompressible fluid)`;
        ...
        if (isLifted) {
          $('telem-val-6').textContent = I18N[state.lang].status_press_lifted;
          $('telem-val-6').className = 'telemetry-value highlight-emerald';
          $('telem-sub-6').textContent = `Δh = +${(state.piston2Height - 90).toFixed(1)} мм • ΔV = +${((state.piston2Height - 90) * S2 / 1000).toFixed(0)} мл`;
        } else if (state.pressSubmode === 'standard' && state.force1 > 0 && F2 < loadWeight) {
          const reqF1 = loadWeight / (S2 / S1);
          $('telem-val-6').textContent = I18N[state.lang].status_press_insufficient_force;
          $('telem-val-6').className = 'telemetry-value highlight-amber';
          $('telem-sub-6').textContent = state.lang === 'uk' ? `Потрібно F₁ ≥ ${reqF1.toFixed(0)} Н (маєте ${state.force1} Н)` : `Required F₁ ≥ ${reqF1.toFixed(0)} N (have ${state.force1} N)`;
        } else {
          $('telem-val-6').textContent = I18N[state.lang].status_press_idle;
          $('telem-val-6').className = 'telemetry-value highlight-indigo';
          $('telem-sub-6').textContent = state.pressSubmode === 'jack' ? `V_масла = ${state.jackOilVolumeInRam.toFixed(0)} мл` : 'F₁ = 0 Н';
        }
      }
    }
  }
  ...
  function renderStandardIndustrialPress(geom) {
    ...
    ctx.beginPath();
    ctx.roundRect(piston1Center - 22, rodTopY - 12, 44, 14, 5);
    ctx.fill();
    ctx.strokeStyle = '#ffffff';
    ctx.lineWidth = 1.5;
    ctx.stroke();
    ...
    ctx.textAlign = 'center';
    ctx.fillText('PRESS ↕', piston1Center, rodTopY - 2);

    // Force arrow on small piston
    if (state.showForces && state.force1 > 0) {
      drawForceArrow(piston1Center, rodTopY - 16, 45, '#ef4444', `F₁ = ${state.force1} Н`);
    } else if (state.showForces && state.force1 === 0 && !state.isDraggingPiston1) {
      ctx.fillStyle = '#94a3b8';
      ctx.font = '600 7.5px JetBrains Mono';
      ctx.fillText(state.lang === 'uk' ? '↓ тягніть мишкою' : '↓ drag with mouse', piston1Center, rodTopY - 16);
    }
  ...
  function renderAutomotiveJackWithReservoir(geom) {
    ...
    // Incompressible Top Stroke Reached Notification Badge over Jack Ram
    if (state.isJackAtMaxStroke) {
      ctx.save();
      ctx.fillStyle = 'rgba(220, 38, 38, 0.92)';
      ctx.strokeStyle = '#fca5a5';
      ctx.lineWidth = 1.5;
      const bannerW = 270;
      const bannerY = bottomY - cylinderH - 42;
      ctx.beginPath();
      ctx.roundRect(piston2Center - bannerW / 2, bannerY, bannerW, 22, 6);
      ctx.fill();
      ctx.stroke();
      ...
      ctx.fillText(
        state.lang === 'uk'
          ? '🔒 ХІД ШТОКА ЗАВЕРШЕНО • РІДИНА НЕСТИСЛИВА'
          : '🔒 MAX STROKE REACHED • FLUID INCOMPRESSIBLE',
        piston2Center,
        bannerY + 14
      );
      ctx.restore();
