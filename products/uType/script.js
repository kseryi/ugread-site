...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    if (p.get('role') === 'student') {
        ...
        const gradeLabel = G.grade === '1'
            ? (G.lang === 'ua' ? '1 клас (Базовий)' : 'Grade 1 (Keys)')
    ...
    b1.innerHTML = G.lang === 'ua'
        ? `<div class="grade-1-badge">1</div>
           <div class="grade-1-content">
               <span class="grade-1-title">1 КЛАС — БАЗОВИЙ ТРЕНАЖЕР</span>
               <span class="grade-1-desc">Базові літери від А до Я без тексту (вправи та ряди клавіатури)</span>
           </div>`
        : `<div class="grade-1-badge">1</div>
           <div class="grade-1-content">
               <span class="grade-1-title">GRADE 1 — KEYBOARD DRILLS</span>
               <span class="grade-1-desc">Basic letter drills from A to Z without text (rows & key sets)</span>
           </div>`;
...
function selectGrade(n) {
    ...
    G.lessonId = null;
    if (Number(n) === 1) {
        $('lessonTitle').textContent = G.lang === 'ua' ? '1 клас — Базові літери (А-Я)' : 'Grade 1 — Basic Letters (A-Z)';
    } else {
