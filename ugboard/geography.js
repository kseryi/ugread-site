function initGeographyPanel() {
  subjectPanel.innerHTML = `
    ...
    <div class="setting-row">
      ...
      <button id="addGlobeBtn" class="ghost-btn">Додати глобус</button>
  ...
  const addGlobeBtn = document.getElementById('addGlobeBtn');
  ...
  // --- Додавання глобуса ---
  addGlobeBtn.addEventListener('click', () => {
    let oldIframe = document.getElementById('geoGlobe');
    if (oldIframe) {
      oldIframe.remove();
      return;
    }
    ...
    iframe.id = 'geoGlobe';
    iframe.src = "../products/globe/globe.html"; // глобус
    ...
    iframe.style.zIndex = "0"; // iframe під полотном для малювання

    boardContainer.appendChild(iframe);

    // Малювання поверх iframe працює, бо drawLayer зверху
  });
